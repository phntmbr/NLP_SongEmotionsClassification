...
namespace LIBSVM_processing
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            sw.Close();
			*/
            sr = new StreamReader(@"C:\Users\Ian Hsieh\Downloads\Project Testsite\MIR_TF_test_Y.txt");
            StreamWriter sw1 = new StreamWriter(@"C:\Users\Ian Hsieh\Downloads\Project Testsite\MIR_TF_test_Y_SVM.txt");
            ...
            foreach (string str in genre1)
            {
                ...
                foreach (string s in r)
                {
                    if (s == null)
                        continue;
                    if (s.Substring(s.IndexOf(':') + 1, s.Length - s.IndexOf(':') - 1) != "0 ")
                    {
                        sw1.Write(s);
                        sw1.Flush();
                    }
